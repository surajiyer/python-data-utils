...
def summary_params(
        ...
        margeff_kwargs={}):
    if not all(isinstance(a, float) and 0. <= a <= 1. for a in significance_levels):
        raise ValueError('significance levels must be in the range [0, 1].')
    if not all(a > b for a, b in zip(significance_levels, significance_levels[1:])):
...
def summary(
    ...
    if 'significance_levels' not in summary_kwargs:
        ...
        summary.add_text(
            'Significance level: ' + ' '.join(['*' * (i + 1) + f': p < {a};' for i, a in enumerate(summary_kwargs['significance_levels'])]))
