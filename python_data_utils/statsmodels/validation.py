...
def scale_location_plot(results, n_annotate=3):
    ...
    fig.axes[0].set_xlabel('Fitted values')
    fig.axes[0].set_ylabel(r'$\sqrt{|Standardized Residuals|}$')
...
def plot_logit_marginal_effects(results, ci=95):
    """
    Plot coefficients and their confidence intervals for a
    statsmodels logit model. Based on (but heavily modified
    and simplified) seaborn's now deprecated coefplot.

    URL: https://github.com/mwaskom/seaborn/blob/master/seaborn/regression.py

    Params
    ------
    results : sm.discrete.discrete_model.L1BinaryResultsWrapper
        Logit model results
    ...
    h, w = mpl.rcParams["figure.figsize"]
    f, ax = plt.subplots(1, 1, figsize=(n_terms * (1 / 2), n_terms * (h / (4 * (n_terms / 5)))))
    for i, _ in enumerate(coef_names):
        low, high = cis[i]
        ax.plot([low, high], [i, i], solid_capstyle="round", lw=2.5,
                color='black')
