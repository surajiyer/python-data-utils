...
class HTML(BaseParser):
    ...
    async def _async_render(self, *, url: str, script: str = None, scrolldown, sleep: int, wait: float, reload, content: Optional[str], timeout: Union[float, int], keep_page: bool):
        ...
        try:
            ...
            async def handle_request(r):
                if r.resourceType in ('image', 'font', 'stylesheet'):
                    await r.abort()
                else:
                    await r.continue_()
    ...
    def render(self, retries: int = 8, script: str = None, wait: float = 0.2, scrolldown=False, sleep: int = 0, reload: bool = True, timeout: Union[float, int] = 8.0, keep_page: bool = False):
        ...
        for i in range(retries):
            if not content:
                try:
                    content, result, page = self.session.loop.run_until_complete(asyncio.wait_for(self._async_render(url=self.url, script=script, sleep=sleep, wait=wait, content=self.html, reload=reload, scrolldown=scrolldown, timeout=timeout, keep_page=keep_page), timeout=timeout + sleep + wait + 10))
                except TimeoutError:
                    content, result, page = None, None, None
                except TypeError:
                    pass
            else:
                break
